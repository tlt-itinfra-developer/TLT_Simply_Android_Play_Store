...
import tlt.th.co.toyotaleasing.modules.custloan.loanmodule.ndid.common.IDPEContractMethodAdapter

class LoanEContractSelectBankNDIDActivity   : BaseActivity() ,  NDIDdialogFragment.Listener , MsgdescNormalDialog.Listener {
    ...
    private val data_description by lazy {
        intent?.getStringExtra(LoanSelectBankNDIDActivity.NDID_DESCRIPTION) ?: ""
    }

    private val viewModel by lazy {
        ViewModelProviders.of(this).get(LoanEContractSelectBankNDIDViewModel::class.java)
    }

    private var dataNDID : LoanEContractSelectBankNDIDViewModel.IDPListModel? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.fragment_loan_car_ndid_select_bank)
        AnalyticsManager.trackScreen(AnalyticsScreenName.ONLINE_BANK_NDID)
        initViewModel()
        initInstance()
    ...
    fun initInstance() {
        ...
        btn_back.setOnClickListener {
            LoanEContractWaitNDIDActivity.start(this@LoanEContractSelectBankNDIDActivity!!, data_extra , "")
        }
    }
    ...
    fun initViewModel(){
        ...
        viewModel.whenDataLoadedStep.observe(this, Observer {
            it?.let {
                if(it != "") {
                    MenuStepController.open(this@LoanEContractSelectBankNDIDActivity, data_extra, it, "")
                }
            }
        })
    ...
    private fun initLayoutManager() {
        ...
        rv_payment_ndid_method_choice.apply {
            layoutManager = GridLayoutManager(this@LoanEContractSelectBankNDIDActivity, 3)
            isNestedScrollingEnabled = false
        }
    }

    private fun initDPListAdapter(dataList: List<LoanEContractSelectBankNDIDViewModel.IDPListModel> = listOf()) {
        status_title_des.visibility = View.GONE
        rv_payment_ndid_method_choice.adapter = IDPEContractMethodAdapter(dataList.toMutableList(), IDPMethodListener)

    }
    private val IDPMethodListener = { position: Int, items: LoanEContractSelectBankNDIDViewModel.IDPListModel ->
        dataNDID = items
        try {
            AnalyticsManager.onlineBankClick(dataNDID?.marketingName.toString())
...
//        finish()
    ...
    companion object {
        ...
        fun start(activity: Activity?, data: String , desc : String , ndid : String = "") {
            val intent = Intent(activity, LoanEContractSelectBankNDIDActivity::class.java)
