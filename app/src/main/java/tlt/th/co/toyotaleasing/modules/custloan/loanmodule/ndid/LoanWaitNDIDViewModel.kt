...
class LoanWaitNDIDViewModel  : BaseViewModel() {
...
//        whenLoading.value = true
        ...
        apiLoanManager.waitNDID(request){ isError, result, step, msg ->
//            whenLoading.value = false
    ...
    fun CheckStepAPI(refID : String , screenName : String)  {
        ...
        apiLoanManager.checkStep(CheckStepRequest.build(refID =  refID  , screen_name = screenName ))  { isError, result, step, msg  ->
                    whenLoading.value = false
                    if (isError) {
                        whenLoading.value = false
                        if (result != "device logon") {
