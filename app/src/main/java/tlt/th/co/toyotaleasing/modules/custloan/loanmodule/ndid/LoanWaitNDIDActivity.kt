...
class LoanWaitNDIDActivity : BaseActivity()  , MsgdescNormalDialog.Listener{
    ...
    private var screen_name : String  = "WAITNDID"

    private val viewModel by lazy {
        ViewModelProviders.of(this).get(LoanWaitNDIDViewModel::class.java)
    }
...
//        toggleLoadingScreenDialog(isEnable = false)
    ...
    private fun initViewModel() {
        ...
        viewModel.whenSyncSuccessData.observe(this, Observer {
            it?.let {
                if (it.step != "WAITNDID") {
                    MenuStepController.open(this@LoanWaitNDIDActivity, it.ref_id, it.step, data_ndid_description , data_ndid)
//                LoanWaitNDIDActivity.start(this@LoanEconsentActivity!!, it.ref_id , it.ref_url)
    ...
    fun initMenus(it: LoanWaitNDIDViewModel.AuthenModel) {
    ...
    companion object {
        ...
        fun start(activity: Activity?, data: String , url : String) {
            val intent = Intent(activity, LoanWaitNDIDActivity::class.java)
