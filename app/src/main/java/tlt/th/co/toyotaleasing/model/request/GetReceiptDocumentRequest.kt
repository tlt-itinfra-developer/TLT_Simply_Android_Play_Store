package tlt.th.co.toyotaleasing.model.request

import com.google.gson.annotations.Expose
import com.google.gson.annotations.SerializedName

class GetReceiptDocumentRequest {

    @SerializedName("PSELECT")
    @Expose
    var pselect: String = ""
    @SerializedName("PKEY1")
    @Expose
    var pkey1: String = ""
    @SerializedName("PKEY2")
    @Expose
    var pkey2: String = ""
    @SerializedName("PKEY3")
    @Expose
    var pkey3: String = ""
    @SerializedName("PKEY4")
    @Expose
    var pkey4: String = ""

    companion object {
        fun build(contractNumber: String = "",
                  documentNumber: String = "",
                  receiptDate : String = ""): GetReceiptDocumentRequest {
            return GetReceiptDocumentRequest().apply {
                pkey1 = contractNumber
                pkey2 = documentNumber
                pkey3 = receiptDate
            }
        }
    }
}